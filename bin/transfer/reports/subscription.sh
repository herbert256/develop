...

{
    ...
    printf 'HEAD\tSubscription\tFiles\tError\tOK\tCured\tVolume\tFirst seen\tLast seen\n'
    printf 'KIND\tsite\tnum\tnumfailed\tnumprocessed\tnumwarn\tnum\ttext\ttext\n'
    printf 'RECALC\t-\ts0\ts1\ts2\ts4\th3\t-\t-\n'
    [ -n "$summary_rows" ] && printf '%s\n' "$summary_rows"
    printf 'TOTAL\tTotal (%s subscription(s))\t@{class=num}%s\t@{class=num failed}%s\t@{class=num processed}%s\t@{class=num warn}%s\t@{class=num}%s\t\t\n' \
    ...
    printf 'HEAD\tSubscription\tDate\tFiles\tError\tOK\tCured\n'
    printf 'KIND\tsite\ttext\tnum\tnumfailed\tnumprocessed\tnumwarn\n'
