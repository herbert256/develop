...

{
    ...
    printf 'HEAD\tSubscription\tFiles\tError\tOK\tVolume\tFirst seen\tLast seen\n'
    printf 'KIND\tsite\tnum\tnumfailed\tnumprocessed\tnum\ttext\ttext\n'
    printf 'RECALC\t-\ts0\ts1\ts2\th3\t-\t-\n'
    [ -n "$summary_rows" ] && printf '%s\n' "$summary_rows"
    printf 'TOTAL\tTotal (%s subscription(s))\t@{class=num}%s\t@{class=num failed}%s\t@{class=num processed}%s\t@{class=num}%s\t\t\n' \
        "$summary_row_count" "$tot_records" "$tot_failed" "$tot_processed" "$tot_human"

    printf 'TABLE\tDetail per Subscription / Date\tgroup\n'
    printf 'HEAD\tSubscription\tDate\tFiles\tError\tOK\n'
    printf 'KIND\tsite\ttext\tnum\tnumfailed\tnumprocessed\n'
