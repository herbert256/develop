...
# date, the per-date base metrics (count:failed:processed:bytes:cured) for the date
...

{
    ...
    printf 'HEAD\tAccount\tFiles\tError\tOK\tCured\tVolume\tFirst seen\tLast seen\n'
    printf 'KIND\tacct\tnum\tnumfailed\tnumprocessed\tnumwarn\tnum\ttext\ttext\n'
    printf 'RECALC\t-\ts0\ts1\ts2\ts4\th3\t-\t-\n'
    [ -n "$summary_rows" ] && printf '%s\n' "$summary_rows"
    printf 'TOTAL\tTotal (%s account(s))\t@{class=num}%s\t@{class=num failed}%s\t@{class=num processed}%s\t@{class=num warn}%s\t@{class=num}%s\t\t\n' \
        "$summary_row_count" "$tot_records" "$tot_failed" "$tot_processed" "$tot_cured" "$tot_human"

    printf 'TABLE\tDetail per Account / Date\tgroup\n'
    printf 'HEAD\tAccount\tDate\tFiles\tError\tOK\tCured\n'
    printf 'KIND\tacct\ttext\tnum\tnumfailed\tnumprocessed\tnumwarn\n'
