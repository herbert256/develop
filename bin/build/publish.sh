...
analyses/subscriptions.html|Subscriptions (analyses)|Every configured subscription on one row: whether it is active, its Logical, Account, Partner, Domain, Application and BL groups, the endpoint (login or remote host), the From and To folders, and the all-time File counts (total, in, out, Errors, automatic retries, resubmits, Waiting, Expired).|mapping, flowid, tags, BL, endpoint, from, to, folders, cron, schedule, counts, files, errors, active, inactive, undeployed, disabled|mapping, BL tag
