...
analyses/subscriptions.html|Subscriptions (analyses)|Every configured subscription on one row: its Logical, Account, Partner, Domain, Application and BL groups, the endpoint (login or remote host), the From and To folders, and the all-time File counts (total, in, out, Errors, automatic retries, resubmits, Waiting, Expired).|mapping, flowid, tags, BL, endpoint, from, to, folders, cron, schedule, counts, files, errors|mapping, BL tag
...
analyses/partners-in.html|Partners - Incoming|Every FE login on one line: the FE overview (use cases, the last logon here and on the old gateway, Files in / out, retrieved, Waiting, Expired, pickups) combined with the Incoming logon funnel (Allowed, Disallowed, Authenticated, No account, Bad key, Key failures, Locked, Auth failed, Session errors, Re-screens, logons and pattern).|partners, incoming, fe, login, funnel, allowed, disallowed, authenticated, bad key, locked, session errors, re-screens, pickups, gateway, migration|
