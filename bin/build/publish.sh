...
analyses/fe-overview.html|FE overview|Every FE login on one line: use cases, the last logon here and on the old gateway, Files in / out with the collected, Waiting and Expired ones, and its pickups with their cadence.|fe, login, overview, status, use case, uc2, uc4, last logon, gateway, migration, files, collected, waiting, expired, pickup, pattern|fe, login, overview, pickup
