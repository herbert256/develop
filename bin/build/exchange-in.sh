...
# request): the INBOX — a git repo, ~/exchange/ by default — for export
# archives, the ONLY intake since 2026-09-12 (the ~/cloud drop is gone), read at the START of the runtime build, before anything parses.
#
...
EXD="${EX/#$HOME/~}"
# the build report's Inbox block (build/inbox.tsv): status ⇥ name ⇥ detail
inbox_note() { [ -d build ] && printf '%s\t%s\t%s\n' "$1" "$2" "$3" >> build/inbox.tsv; return 0; }
if [ -z "$ENV_INBOX" ]; then
    echo "inbox: input/environment.txt says '${ENV_LABEL:-<missing>}' — no inbox prefix for it (Acceptance or Production expected); the inbox is not read." >&2
...
if [ ! -d "$EX/.git" ]; then
    echo "inbox: no git repo at $EXD — skipping (clone the inbox repo there to enable)." >&2
    inbox_note skipped "" "no git repo at $EXD"
    exit 0
fi

if ! git -C "$EX" pull --rebase --autostash --quiet 2>/dev/null; then
    echo "inbox: WARNING - git pull failed (offline? a conflict?) — nothing ingested this build." >&2
...
if [ ${#updates[@]} -eq 0 ]; then
    echo "inbox: pulled — no $pfxs to ingest ($ENV_LABEL)." >&2
    inbox_note none "" "pulled: no $pfxs"
    exit 0
fi
...
for f in "${updates[@]}"; do
    ...
    if bin/build/st-reports-update.sh "$f"; then
        ...
        echo "inbox: WARNING - $rel was NOT ingested; it stays in the inbox — fix or remove it, then rebuild." >&2
        failed+=("$rel")
    fi
done
