...
# this repo's ONE environment, synthetic and DETERMINISTIC (seed
# AXWAY_SAMPLE_SEED, default 42 — re-running reproduces byte-identical files;
# a different seed gives a different but equally valid estate).
#
#   bin/sample/generate.sh            # no arguments — one repo, one estate
#
# ONE estate since 2026-09-11 (one repo = one environment): the roster is the
# UNION of the two former sample environments (bin/sample/estate.awk), so no
# planted scenario was lost when the environment level went.
#
# Writes:  input/flow-manager/{partners,subscriptions}.json,
#   input/{transfer,server}/*.csv (one file per day, newest-first rows),
#   input/renames/{subscriptions,profiles,flowid-names}.tsv,
#   input/ip/ip-hosts.tsv, input/.sample/ (estate spec + expected figures)
#   — plus the READMEs and the ten policy files (from bin/sample/templates/).
#   NOT written: input/environment.txt (the checkout's own label — see
#   bin/envlabel.sh) and input/.sample-estate (the marker below).
#
...
echo "sample: renames + ip sidecars ..." >&2
...
echo "sample: server CSVs ..." >&2
rm -f "$IN"/server/*.csv
LC_ALL=C sort -t"$(printf '\t')" -k2,2nr "$IN/.sample/_events.tsv" \
    | sawk -F'\t' -f "$SCRIPT_DIR/gen-server.awk" -v DIR="$IN/server"

rm -f "$IN/.sample/_events.tsv"
printf 'sample: %s transfer rows, %s server rows.\n' \
    "$(cat "$IN"/transfer/*.csv | wc -l | tr -d ' ')" \
    "$(cat "$IN"/server/*.csv | wc -l | tr -d ' ')" >&2

# ---- the policy files (the sample templates; the runtime repos keep their own)
for f in blacklist.txt skip.txt rename.txt logical.txt logical_domains.txt logical_apps.txt logical_partners.txt BL.txt logons_old.txt coreid-url.txt; do
    cp "$TPL_DIR/$f" "input/$f"
done

cat > input/README.txt <<'EOT'
...
cat > "input/flow-manager/README.txt" <<'EOT'
...
EOT
cat > "input/transfer/README.txt" <<'EOT'
...
EOT
cat > "input/server/README.txt" <<'EOT'
...
EOT
cat > "input/renames/README.txt" <<'EOT'
