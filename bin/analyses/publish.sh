...
write_analyses_index() {
    local out="$ADIR/index.html"
    {
        ...
        [ -f "$ADIR/partners-in.html" ] && printf '<tr><td><a href="partners-in.html">Partners - Incoming</a></td><td class="desc">Every FE login on one line: the FE overview (use cases, the last logon here and on the old gateway, Files in / out, retrieved, Waiting, Expired, pickups) combined with the Incoming logon funnel (Allowed, Disallowed, Authenticated, No account, Bad key, Key failures, Locked, Auth failed, Session errors, Re-screens, logons and pattern).</td></tr>\n'
