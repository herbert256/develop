...
write_analyses_index() {
    local out="$ADIR/index.html"
    {
        ...
        [ -f "$ADIR/fe-overview.html" ] && printf '<tr><td><a href="fe-overview.html">FE overview</a></td><td class="desc">Every FE login on one line: its use cases and subscriptions, the last logon here and on the old gateway, its Files in / out with the Waiting and Expired ones, and what its UC2 pickup visits did &mdash; pickups, collected, two-way, delivery-only and same-connection.</td></tr>\n'
        [ -f "$ADIR/fe-status.html" ] && printf '<tr><td><a href="fe-status.html">FE status information</a></td><td class="desc">Every FE login on one line: its use cases (UC2, UC4 or the UC2/UC4 mailbox pair), the last logon here, the last logon on the old gateway, and its Files in / out with the Waiting ones separately.</td></tr>\n'
