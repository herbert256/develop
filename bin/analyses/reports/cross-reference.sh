#!/usr/bin/env bash
#
# cross-reference.sh — Entities Cross References: WHICH pairs of the seven
# entities (account, login, subscription, remote host, partner, application,
# domain) belong together — an ANALYSIS
...
# (render_report clears CUR_DATES for cross-*). Writes seven
# data files —
#   cross-account.rpt  cross-login.rpt  cross-subscription.rpt
#   cross-host.rpt
#   cross-partner.rpt  cross-application.rpt  cross-domain.rpt
# — each holding six tables (the other six entities), which publish_lib.sh
# splits into 42 pages. The page's two nav rows are the two selectors: row 1
...
# inherits its File's partner / application / domain attribution
# (cols 20 / 18 / 19).
...
# 21 unordered combinations are computed once and emitted into both
# orientations.
#
# Usage:
...
ENTS="acct login site host ptn app dom"

ent_rpt()   { case $1 in acct) echo cross-account;; login) echo cross-login;; site) echo cross-subscription;; host) echo cross-host;; ptn) echo cross-partner;; app) echo cross-application;; dom) echo cross-domain;; esac }
ent_tab()   { case $1 in acct) echo "Account";; login) echo "Login";; site) echo "Subscriptions";; host) echo "Hosts";; ptn) echo "Partners";; app) echo "Applications";; dom) echo "Domains";; esac }
ent_col()   { case $1 in acct) echo "Account";; login) echo "Login";; site) echo "Subscription";; host) echo "Remote Host";; ptn) echo "Partner";; app) echo "Application";; dom) echo "Domain";; esac }
ent_kind()  { case $1 in acct) echo acct;; login) echo login;; site) echo site;; host) echo host;; ptn) echo ptn;; app) echo app;; dom) echo dom;; esac }   # every entity type links to its detail pages
ent_xref()  { case $1 in acct) echo accounts;; login) echo logins;; site) echo subscriptions;; host) echo hosts;; ptn) echo partners;; app) echo apps;; dom) echo domains;; esac }   # data/flow-manager/xref item names
ent_base()  { case $1 in acct) echo _accounts;; login) echo _logins;; site) echo _subscriptions;; host) echo _hosts;; ptn) echo _partners;; app) echo _apps;; dom) echo _domains;; esac }   # base cache (name/direction/result) per entity
...
agg=$(awk -F'\t' -v SPMAP="$CONFIG_XREF/_subscriptions-partners.tsv" -v APMAP="$CONFIG_XREF/_accounts-apps.tsv" '
    ...
    function ujoin(v6, k6, M6,   n6, i6, r6) {
        ...
        if (k6 != "" && (toupper(k6) in M6)) { n6 = split(M6[toupper(k6)], UZ6, "\037")
            for (i6 = 1; i6 <= n6; i6++) if (index("\037" r6 "\037", "\037" UZ6[i6] "\037") == 0)
                r6 = r6 (r6 == "" ? "" : "\037") UZ6[i6] }
        return r6 }
    BEGIN { split("acct login site host ptn app dom", E, " ")
        ...
        # parse keeps one)
        uload(SPMAP, sp); uload(APMAP, ap2) }
        ...
        next }
    {
        ...
        for (p6 = 1; p6 <= np6; p6++) for (a6 = 1; a6 <= na6; a6++) {
            V["ptn"] = PT6[p6]; V["app"] = AP6[a6]
            for (i = 1; i <= 6; i++) for (j = i + 1; j <= 7; j++) {
                va = V[E[i]]; vb = V[E[j]]
                if (va == "" || vb == "") continue
...
#   awk #2  loads the 7 base result caches + the 4 data/unknown seed lists
...
printf '%s\n' "$agg" | awk -F'\t' -v OFS='\t' -v XD="$CONFIG_XREF" -v XREFS="$XREFS" '
    BEGIN { ne = split("acct login site host ptn app dom", E, " ")
            split(XREFS, XR, " ")
...
' | LC_ALL=C sort -t"$(printf '\t')" -k1,1 -k2,2 -k6,6 -k7,7 -k4,4 -k5,5 \
  ...
  | awk -F'\t' -v CB="$CONFIG_BASE" -v UD="$UNKNOWN_DIR" -v TMP="$TMP" \
    ...
    BEGIN {
        ne = split("acct login site host ptn app dom", E, " ")
