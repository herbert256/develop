...
the sample template), `logons_old.txt` (2026-09-02: the FE logins' last logon on the OLD gateway, `<login> <stamp>` per line — the Analyses → Configuration "FE status information" page's Gateway logon column; hand-maintained, sample template in develop) and
